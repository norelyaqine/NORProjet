...
abstract class DBDataWrapper extends DataWrapper{
	...
	*/
	protected function select_query($select,$from,$where,$sort,$start,$count){
	 ...
	 $name = $_GET['projet'];	           
		$sql="SELECT * FROM gantt_tasks  where projet = '".$name."' ";
		return $sql;
	}
	...
	*/
	protected function insert_query($data,$request){
		...
		if($request->get_source()=="gantt_tasks"){
				
		$sql="INSERT INTO gantt_tasks(start_date,duration,text,progress,sortorder,parent,id,projet) VALUES (".implode(",",$temp_v).",".$na.")";
		}else
